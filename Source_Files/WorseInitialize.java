  ...
  public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
      if (c) {
          if (onSliderStatecListener != null) {
              onSliderStatecListener.onPagecd(findViewById(12345 + position)(mP.getCurrentItem() + (position < mP.getCurrentItem() ? -1 : 1)), sliderState);
              c = !c;
          }
      }
  }
...
getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
    ...
    public void onGlobalLayout() {
        ...
        if (onSliderStatecListener != null) {
            onSliderStatecListener.onPagecd(findViewById(12345 + position)(mP.getCurrentItem()), sliderState);
        }
    }
});
    }